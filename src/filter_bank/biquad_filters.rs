...
impl BiquadFilters {
  ...
  pub fn process_simd(
    ...
    (b1_re.mul_add(input, z2_re) - a1_re * y_re + a1_im * y_im).copy_to_slice(&mut self.z1_re[i..]);
    (b1_im.mul_add(input, z2_im) - a1_re * y_im - a1_im * y_re).copy_to_slice(&mut self.z1_im[i..]);
  ...
  pub fn process_scalar(&mut self, input: f32, i: usize) -> (f32, f32) {
    ...
    self.z1_re[i] =
      self.b1_re[i].mul_add(input, self.z2_re[i]) - self.a1_re[i] * y_re + self.a1_im[i] * y_im;
    self.z1_im[i] =
      self.b1_im[i].mul_add(input, self.z2_im[i]) - self.a1_re[i] * y_im - self.a1_im[i] * y_re;
